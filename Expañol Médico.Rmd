...
p_net_lecture <- ggraph(net_tag, layout = 'stress') +
  geom_edge_arc0(aes(width = width/10), linetype = 1, color="gray60", strength = 0.3, alpha = 0.5) +
  geom_node_point(color = lec_color, size = 15) +
  geom_node_text(aes(label = str_wrap(name, 10)), vjust = 0.5, size = 1.6) +
  expand_limits(x = c(-0.75, 0.75), y = c(-1, 1)) +
  theme( 
    legend.position = "none",
    plot.background = element_rect(size = 10),
    plot.margin = margin(rep(1, 4), "cm")
    )
